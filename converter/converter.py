...
layer_type_mapping = {'InputLayer': 'Input', 'ReLU': 'ReLU', 'LeakyReLU': 'ReLU',
...
def get_in_out_string(in_dict):
    ...
    split_info = in_dict['split_info']
    split_info_add = {}

    split_string = None
    if len(layer.outbound_nodes) > 1:
        split_info_add = {}
        split_postfix = str(len(split_info)).zfill(3)

        layer_type = 'Split'
        layer_name = f'{layer.name}_{split_postfix}'
        split_output_blobs = []
        split_output_blobs_dict = {}
        for split_index, outbound_layer in enumerate(layer.outbound_nodes):
            split_blob_src_name = outbound_layer.outbound_layer.name
            split_blob_dst_name = split_blob_src_name + f'_split_{str(split_index).zfill(3)}'
            split_output_blobs.append(split_blob_dst_name)
            split_output_blobs_dict[split_blob_src_name] = split_blob_dst_name
        split_info_add[layer.name] = split_output_blobs_dict

        split_input_blobs = [layer.name]
        in_out_list = [str(len(split_input_blobs)), str(len(layer.outbound_nodes)), ' '.join(split_input_blobs), ' '.join(split_output_blobs)]

        in_out_string = ' '.join(in_out_list)
        # split_string = f'{layer_type: <24}{layer_name: <24}{in_out_string}'
        batch_size = in_dict['batch_size']
        if isinstance(layer.output_shape,list):
            assert len(layer.output_shape) == 1, 'we did not suport this!!!'
            _layer_output_shape = layer.output_shape[0]
        else:
            _layer_output_shape = layer.output_shape

        blob_shape_string = get_split_shape_string([_layer_output_shape]*len(layer.outbound_nodes), batch_size)
        array_key = '-23330'
        split_string = f'{layer_type: <24}{layer_name: <24}{in_out_string} {array_key}={blob_shape_string}'

        assert len(input_blobs) < 2, f'We did not support this {input_blobs}!!!'

        output_blobs = [layer_name]

    split_info.update(split_info_add)
    input_blobs_copy = []
...
def get_relu_mapping(in_dict):
    # ReLU	0	slope	0.f
    slope = float("{0:.7f}".format(float(0)))
