...
info_ReLU_max = 'ReLU: Max Clip is not supported withing NCNN, split by ordinal ReLU and Clip'

def transfer_ReLU_max(src_model, dst_model, transfer_rule):
    pass

def detect_transform_ReLU_max(keras_config):
    index_list = []
    for i, item in enumerate(keras_config['layers']):
        if item['class_name'] == 'ReLU':
            if 'max_value' in item['config']:
                if item['config']['max_value'] is not None:
                    index_list.append(i)
    return index_list

def check_ReLU_max_transfrom(keras_config):
    return len(detect_transform_ReLU_max(keras_config)) > 0

def apply_transform_ReLU_max(keras_config):
    index_list = detect_transform_ReLU_max(keras_config)
    ...
    while len(index_list) > 0:
        ...
        weight_transfer_rule_dict[i_layer_config['name']] = {'transfer_call': transfer_ReLU_max,
        ...
        for key in i_layer_config['config'].keys():
            if key in r_layer_config['config']:
                i_layer_config['config'][key] = r_layer_config['config'][key]
        i_layer_config['config']['max_value'] = r_layer_config['config']['max_value']
        i_layer_config['config']['min_value'] = r_layer_config['config']['threshold']
