...
info_Conv2DSigmoid = 'Conv2D->Sigmoid: Inline operations should be merged'


def transfer_Conv2DSigmoid_Conv2D(src_model, dst_model, transfer_rule):
    dst_model.get_layer(transfer_rule['dst']).set_weights(src_model.get_layer(transfer_rule['src']).get_weights())


def detect_transform_Conv2DSigmoid(keras_config):
    index_list = []
    outbound_dict, index_dict = get_outbound_nodes(keras_config)
    for i, item in enumerate(keras_config['layers']):
        if item['class_name'] == 'Sigmoid':
            ...
            if in_node_class_name in ['Conv2D', 'DepthwiseConv2D', 'Conv2DTranspose']:
                ...
                if _activation in ['linear', 'sigmoid']:
                    if len(outbound_dict[in_node_name]) == 1:
                        index_list.append(i)
    return index_list, index_dict


def check_Conv2DSigmoid(keras_config):
    return len(detect_transform_Conv2DSigmoid(keras_config)[0]) > 0


def apply_transform_Conv2DSigmoid(keras_config):
    index_list, index_dict = detect_transform_Conv2DSigmoid(keras_config)
    ...
    while len(index_list) > 0:
        ...
        keras_config['layers'][index_dict[dst_name]]['config']['activation'] = 'sigmoid'
        transfer_call = transfer_Conv2DSigmoid_Conv2D
